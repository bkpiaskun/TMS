...
-- Tworzenie tymczasowej tabeli aby przezwyciężyć błędy z zależnościami w WIDOKU
CREATE TABLE `RAW_Last_Measurements` (
	`ID` INT(11) NOT NULL,
	`Sensor_ID` INT(11) NOT NULL,
	`Sensor_Owner` INT(11) NOT NULL,
	`Sensor_Name` VARCHAR(30) NOT NULL COLLATE 'utf8_general_ci',
	`Timestamp_Of_Reading` TIMESTAMP NOT NULL,
	`AVG_Humidity` FLOAT NULL,
	`Max_Humidity` FLOAT NULL,
...
-- Zrzut struktury tabela TemperatureMeasureSite.Sensor_Log
...
CREATE TABLE IF NOT EXISTS `Sensor_Readings` (
  ...
  PRIMARY KEY (`ID`),
  KEY `Sensor_ID` (`Sensor_ID`)
) ENGINE=InnoDB AUTO_INCREMENT=425669 DEFAULT CHARSET=utf8;
...
-- Zrzut struktury procedura TemperatureMeasureSite.Users_Last
DELIMITER //
CREATE DEFINER=`sensorsite`@`%` PROCEDURE `Users_Last`(
...
concat(if((hour(`temptable1`.`diffTime`) > 0),concat(hour(`temptable1`.`diffTime`),' godzin '),''),if((minute(`temptable1`.`diffTime`) > 0),concat(minute(`temptable1`.`diffTime`),' minut '),''),second(`temptable1`.`diffTime`),' sekund temu') AS `Timestamp_Of_Reading`,
...
from (
    ...
    from (   
		   ...
		   JOIN Users us
		   ON rlm.Sensor_Owner = us.User_ID
